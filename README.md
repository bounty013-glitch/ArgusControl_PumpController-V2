...
**Current Status**: Phase 4D.3 is **COMPLETE AND ACCEPTED** as of July 23, 2026. Firmware identity is `v2-phase4d.3-dev`. Human browser access is SoftAP-only and uses strict local login, bounded RAM-only sessions, CSRF, deny-by-default capability authorization, human-account/custom-role administration, bounded security audit, protected active AP credential management, and authenticated physical-recovery exit. Legacy browser Basic Auth is retired. Encryption still uses software-stored XTS keys and does not resist physical flash extraction; HTTP remains plaintext, and MQTT authentication, HTTPS/TLS, certificates, hostile-network acceptance, and irreversible device-security provisioning remain deferred to later gates. Phase 4C motion behavior and its single router path remain intact; no powered-motion or process acceptance is implied by Phase 4D.3. See [Phase 4D.3 Plan](docs/PHASE_4D_3_IMPLEMENTATION_PLAN.md) and [Phase 4D.3 Acceptance Record](docs/Phase%204D.3%20Tests.md).
