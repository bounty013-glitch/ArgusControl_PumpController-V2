...
**Current Status**: Phase 4D.3a is **COMPLETE AND ACCEPTED** as of July 23, 2026. Firmware identity is `v2-phase4d.3a-dev`. The Phase 4D.3 browser-security architecture remains intact, with truthful prepared/terminal audit mutation outcomes, response-before-transition ordering for AP-password and recovery-exit operations, complete registered human-route enforcement, removal of retired `/api/logout`, and real bounded audit pagination. Legacy browser Basic Auth remains retired. Encryption still uses software-stored XTS keys and does not resist physical flash extraction; HTTP remains plaintext, and MQTT authentication, HTTPS/TLS, certificates, hostile-network acceptance, and irreversible device-security provisioning remain deferred to later gates. Phase 4C motion behavior and its single router path remain intact; no powered-motion or process acceptance is implied. See [Phase 4D.3a Plan](docs/PHASE_4D_3A_IMPLEMENTATION_PLAN.md) and [Phase 4D.3a Acceptance Record](docs/Phase%204D.3a%20Tests.md).
