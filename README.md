...
**Current Status**: Phase 4D.4 is **COMPLETE AND ACCEPTED** as of July 25, 2026. Firmware identity is `v2-phase4d.4-dev`. Phase 4D.4 adds durable bounded machine enrollment, one-time controller-generated credentials, strict MQTT CONNECT authentication, connection-bound principals, per-packet authorization, and immediate rotation/revocation invalidation without changing the accepted Phase 4C router or fail-operational motion architecture. Human browser sessions remain separate from machine identities, and authentication does not grant operating authority. Encryption still uses software-stored XTS keys and does not resist physical flash extraction; HTTP and MQTT remain plaintext, while HTTPS/TLS, certificates, hostile-network acceptance, and irreversible device-security provisioning remain deferred. Acceptance was stationary with the motor physically disconnected; no powered-motion or process acceptance is implied. See [Phase 4D.4 Plan](docs/PHASE_4D_4_IMPLEMENTATION_PLAN.md) and [Phase 4D.4 Acceptance Record](docs/Phase%204D.4%20Tests.md).
