...
**Current Status**: Phase 4B.1 through Phase 4B.3, including the Phase 4B.3a Wi-Fi observability and recovery close-out, are complete and physically accepted. Phase 4B.4 Step 1 and Step 2 are `SOFTWARE-AND-AUTOMATED-RUNTIME-ACCEPTED` at implementation commit `1b701e5` with evidence commit `5dbaf31`. Phase 4B.5 firmware identity is `v2-phase4b.5-dev`; the dedicated authenticated `/controls` page and live authoritative status display are implemented and software-validated at commit `594445b`. Motor-disconnected controller validation is pending positive physical verification that STEP/DIR/ENA are electrically isolated from the driver. Previously accepted lower-layer motor behavior is not reopened, and the narrow powered UI-to-motor confirmation remains deferred. No pump, hose, chemical, pressure, flow-accuracy, process, or mechanical-endurance acceptance is implied.
