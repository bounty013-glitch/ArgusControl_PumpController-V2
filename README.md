...
**Current Status**: Phase 4D.2 is **COMPLETE AND ACCEPTED** as of July 22, 2026. Firmware identity is `v2-phase4d.2-dev`. The controller now has versioned encrypted security storage, PBKDF2-HMAC-SHA-256 verifier and provisioning seams, bounded legacy migration, and a physically validated GPIO0/KEY1 AP-only recovery path. Encryption uses software-stored XTS keys and therefore does not resist physical flash extraction. Browser sessions, full authorization, MQTT authentication, HTTPS/TLS, certificates, and irreversible device-security provisioning remain deferred. Phase 4C motion behavior remains intact; no powered-motion or process acceptance is implied by Phase 4D.2. See [Phase 4D.2 Plan](docs/PHASE_4D_2_IMPLEMENTATION_PLAN.md) and [Phase 4D.2 Acceptance Record](docs/Phase%204D.2%20Tests.md).
