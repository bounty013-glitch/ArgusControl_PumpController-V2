...
**Current Status**: Phase 4D.1 is **COMPLETE AND ACCEPTED** as a security-contract and repository-audit subphase on July 22, 2026. Firmware identity is `v2-phase4d.1-dev`; runtime security behavior remains the accepted Phase 4C implementation. The contract defines separate AP and console credentials, human and machine identities, roles and permission ceilings, AP-only browser sessions, recovery and audit boundaries, and truthful HTTP/non-TLS limitations. It does not implement credentials, sessions, authorization, MQTT authentication, HTTPS/TLS, certificates, or recovery actions. Phase 4C remains fully hardware verified; no new motion or process acceptance is implied. See [Phase 4D Security Contract](docs/PHASE_4D_SECURITY_CONTRACT.md) and [Phase 4D.1 Audit Record](docs/Phase%204D.1%20Tests.md).
