...
**Current Status**: Phase 4B.1 through Phase 4B.3, including the Phase 4B.3a Wi-Fi observability and recovery close-out, are complete and physically accepted. Phase 4B.4 Step 1 and Step 2 are `SOFTWARE-AND-AUTOMATED-RUNTIME-ACCEPTED` at implementation commit `1b701e5` with evidence commit `5dbaf31`. Phase 4B.5 is active with firmware identity `v2-phase4b.5-dev`; its first functional step is the dedicated authenticated browser-controls page and live authoritative status display. Previously accepted lower-layer motor behavior is not reopened, and the narrow powered UI-to-motor confirmation remains deferred. No pump, hose, chemical, pressure, flow-accuracy, process, or mechanical-endurance acceptance is implied.
