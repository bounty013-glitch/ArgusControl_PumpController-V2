...
Each MQTT client authenticates as an enrolled machine identity before subscribe, heartbeat, or publish admission. Per-client policy defines exact allowed interface, subscribe filters, heartbeat permission, command permissions, and topic scope. Authentication and permission rejection occur before Phase 4C topic policy, retained storage, subscriber delivery, heartbeat mutation, sequence consumption, or dispatch.

Phase 4D.4 accepts strict MQTT 3.1.1 username/password authentication against verifier-only machine records, accepted-socket interface enforcement, connection-bound principals, per-packet authorization, and immediate mutation invalidation. The broker still binds port 1883 on active interfaces so policy enforcement remains mandatory. Non-TLS MQTT credentials are observable to a network attacker; this is accepted only within the documented deployment boundary, not as a solved hostile-network risk.
...
Security-sensitive administrative mutations persist a non-success `PREPARED` event before commit and a correlated terminal success or failure afterward. If durable prepared evidence is unavailable, those mutations fail closed. If terminal finalization fails after a committed mutation, the prepared evidence remains detectable and later privileged mutations fail closed until reboot; no terminal result is fabricated and motion is untouched. Authorized audit reads use strict newest-first pages with a maximum of 16 records and an exclusive non-secret sequence cursor. Software E-stop, logout, and safety actions are never delayed or reversed by audit failure; they emit best-effort bounded diagnostics and a storage-failure indication.
...
Phase 4D.2 implemented encrypted security storage, verifier/provisioning foundations, bounded migration, and the physically local recovery trigger. Phase 4D.3 implemented and accepted local browser authentication, human accounts, roles, RAM-only sessions, CSRF, authorization, audit storage, active AP-password mutation, and authenticated recovery exit. Phase 4D.3a accepted truthful prepared/terminal audit outcomes, response-before-transition ordering, complete human-route enforcement, retired-route removal, and bounded audit pagination. Phase 4D.4 accepted bounded machine enrollment, MQTT CONNECT authentication, per-packet authorization, and immediate credential-lifecycle invalidation while preserving Phase 4C authority and routing. HTTPS, TLS, certificates, firmware-management implementation, calibration, hostile-network operation, and penetration acceptance remain deferred. Phase 4D performs no pump, hose, fluid, chemical, pressure, flow, load, process, or endurance acceptance.
