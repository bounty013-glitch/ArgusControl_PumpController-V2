...
10. **Security Contract and Foundation (Phase 4D.1 and 4D.2)**:
    ...
    *   Stores versioned role, human, machine, AP-secret, console-verifier, migration, and recovery metadata in a dedicated encrypted NVS partition with atomic dual-slot selection and fail-closed corruption behavior.
    *   Uses PBKDF2-HMAC-SHA-256 verifiers with random salts, bounded cost, constant-time comparison, and a cooperative KDF worker; plaintext secrets are accepted only through bounded provisioning/migration seams.
    ...
    *   Software-stored XTS keys protect against casual plaintext inspection but remain physically extractable. eFuse/HMAC key derivation, flash encryption, secure boot, browser sessions, MQTT machine authentication, HTTPS/TLS, certificates, and hostile-network operation remain deferred.
